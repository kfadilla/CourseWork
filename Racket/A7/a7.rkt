...
(define (lex exp env)
  (match exp
    ...
    [`(let/cc ,k ,body) `(let/cc ,(lex body (cons k env)))]
...
(define value-of-cps
  (lambda (expr env-cps k)
    (match expr
      ...
      ;[`(let/cc ,body) (value-of-cps body (extend-env k env-cps) k)] let/cc case
      [`(throw ,k-exp ,v-exp) (value-of-cps k-exp env-cps (throw-outer-k v-exp env-cps))]
      [`(let ,e ,body) (value-of-cps e env-cps (let-constructor-k body env-cps k))]
...
;some test
(check-expect (value-of-cps '(const 10) (empty-env) (empty-k)) 10)
(check-expect (value-of-cps '(mult (const 10) (const 5)) (empty-env) (empty-k)) 50)
(check-expect (value-of-cps '(zero (const 0)) (empty-env) (empty-k)) #t)
(check-expect (value-of-cps '(if (zero (const 0)) (const 3) (mult (const 1) (const 2))) (empty-env) (empty-k)) 3)
(check-expect (value-of-cps '(app (lambda (var 0)) (const 10)) (empty-env) (empty-k)) 10)
;(check-expect (value-of-cps '(let/cc (throw (var 0) (mult (const 10) (const 5)))) (empty-env) (empty-k)) 50) let/cc case
